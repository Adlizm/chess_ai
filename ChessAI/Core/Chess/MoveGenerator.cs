...
namespace Core.Chess {
    public static class MoveGenerator {
        static public readonly int[] TOWER_MOVES_OFFSETS    = new int[4] { -1, 1, -8, 8 };
        static public readonly int[] BISHOP_MOVES_OFFSETS   = new int[4] { -9, -7, 7, 9 };
        static public readonly int[] QUEEN_MOVES_OFFSETS    = new int[8] { -1, 1, -8, 8, - 9, -7, 7, 9 };
        ...
        static public List<Move> GetBishopMoves(Board board, GameConfig config, byte localPiece) {
            ...
            for(int j = 0; j < BISHOP_MOVES_OFFSETS.Length; j++) {
                int moveOffSet = BISHOP_MOVES_OFFSETS[j];
                int step = steps[j];
        ...
        static public List<Move> GetTowerMoves(Board board, GameConfig config, byte localPiece) {
            ...
            for(int j = 0; j < TOWER_MOVES_OFFSETS.Length; j++) {
                int moveOffSet = TOWER_MOVES_OFFSETS[j];
                int step = steps[j];
        ...
        static public List<Move> GetQueenMoves(Board board, GameConfig config, byte localPiece) {
            ...
            int[] steps = new int[8] { left, right, top, bottom, 
                                       Math.Min(left, top),    Math.Min(right, top), 
                                       Math.Min(left, bottom), Math.Min(right, bottom) };

            for(int j = 0; j < QUEEN_MOVES_OFFSETS.Length; j++) {
                int moveOffSet = QUEEN_MOVES_OFFSETS[j];
                int step = steps[j];
