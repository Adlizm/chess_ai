...
namespace Core.Chess {
    ...
    public class Game {
        ...
        private List<Board> positions; // Verify draw: 3 equals positions
        ...
        static public Game CreateGame(string displayGame) {
            ...
            game.gameState = game.CalculateGameState();
        ...
        public uint Rooks {
            get => this.gameConfig.Rooks;
        }
        public EnPassant EnPassant {
            get => this.gameConfig.EnPassant;
        }
        ...
        //Try make a move, if is a invalid move then return false, otherwise return true.
        ...
        //Only validate if the piece selected is the same color of current player 
        //Make a move without move validation
        public void UnsafeMakeMove(Move mymove) {
            Piece piece = this.currentPosition[mymove.selected];
            if(!piece.isSameColor(this.gameConfig.TimeOf)) {
                Console.WriteLine("Treid move a enemy piece");
                return;
